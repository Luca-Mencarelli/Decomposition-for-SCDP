...
Pricing problem
"""
function Pricing(modelp,Thetamax,CoverageSatLat,CoverageSatLong,dualvar,EDGE)

# Variables de linéarisation
	@variable(modelp, Xi[1:1,1:n_0,1:NUM_PIXEL], Bin)
# Fonction objectif : Minimize the number of active satellite
	@objective(modelp, Min, sum(dualvar[j,k] * (sum(Xi[i,k,j] for i in [1] if EDGE[i,k,j]==1)) for j in 1:NUM_PIXEL for k in 1:n_0))
# Somma su indici delle orbite
	@constraint(modelp, sum((Xi[i,k,j]) for i=[1] for j in 1:NUM_PIXEL for k in 1:n_0 if EDGE[i,k,j]==1) >= 1)
	optimize!(modelp)
	#=
	Xis = value.(modelp[:Xi])
	if sum(Xis) != 0
...
"""
function RMS(model,INDEX,EDGE)
	sizeIndex = length(INDEX)
# Binary activation of satellite
	@variable(model, activSat[1:sizeIndex])
	@variable(model, 0 <= Xi[1:sizeIndex, 1:n_0, 1:NUM_PIXEL] <= 1)
# Fonction objectif : Minimize the number of active satellite
	@objective(model, Min, sum(activSat[i] for i in 1:sizeIndex))
	@constraint(model, observable[j=1:NUM_PIXEL,k=1:n_0], -sum(Xi[i,k,j] for i in 1:sizeIndex if EDGE[i,k,j]==1) <= -1)
	@constraint(model, [i=1:sizeIndex,k=1:n_0,j=1:NUM_PIXEL], Xi[i,k,j] <= activSat[i])

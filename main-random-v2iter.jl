...
#			end
			...
			global model,u = RMS(model,INDEX,Thetamax,CoverageSatLat,CoverageSatLong)
			...
			for i in shuffle(1:NbrTOTALsat)
				if i ∉ INDEX_PRICING
					...
					global modelp = Pricing(modelp,ThetamaxTemp,CoverageSatLatTemp,CoverageSatLongTemp,u)
...
global name_file = "Results/Global/"*string((T_terre/(n_0*3600)))*"H_RevisitTime/ObjectiveThreshold"*string(ObjectiveThreshold)*
                "_SizeAddOrbitals"*string(SizeAddOrbitalsCG)*"_Nb_Cibles"*string(NUM_PIXEL)*"_TimeStep"*string(dt)*"_percentage"*string(percentage_orbits)*"-v2-iter.txt"
println(name_file)
